...
resource "aws_ecr_repository" "price" {
  name                 = "${var.project_name}-${var.environment}-price"
  image_tag_mutability = "MUTABLE"
  force_delete         = true
...
resource "aws_ecr_lifecycle_policy" "price" {
  repository = aws_ecr_repository.price.name
